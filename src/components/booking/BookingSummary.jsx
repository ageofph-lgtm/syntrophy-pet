...
export default function BookingSummary({ pet, services, professional, date, time, endTime, duration, totalPrice, groomingPrefs }) {
  return (
    <div className="bg-white border border-stone-200 rounded-2xl overflow-hidden shadow-sm">
      <div className="bg-gradient-to-r from-violet-50 to-transparent p-5 border-b border-stone-100">
      ...
      <div className="p-5 space-y-4">
        ...
        {groomingPrefs?.corpo && (
          <div className="bg-violet-50/60 rounded-xl p-3 border border-violet-100">
            <p className="text-[10px] font-semibold text-violet-600 uppercase tracking-wider mb-2">Tosquia</p>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-stone-900">{date ? format(date, "d 'de' MMMM, yyyy", { locale: pt }) : "—"}</p>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-stone-100">
          ...
          <div className="flex items-center gap-1">
            <span className="text-2xl font-bold text-stone-950">{totalPrice?.toFixed(2)}€</span>
          </div>
        </div>
      </div>
    </div>
