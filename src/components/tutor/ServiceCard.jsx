...
const CATEGORY_ICONS = { banho: Droplets, tosquia: Scissors, spa: Sparkles, adicional: Plus };
const CATEGORY_COLORS = {
  banho:    "text-blue-500 bg-blue-50 border-blue-100",
  tosquia:  "text-violet-600 bg-violet-50 border-violet-200",
...
export default function ServiceCard({ service, selected, onToggle }) {
  ...
  return (
    <button onClick={() => onToggle(service)}
      className={`relative w-full text-left bg-white border rounded-2xl p-4 transition-all duration-200 shadow-sm
        ${selected ? "border-stone-950 ring-2 ring-stone-200 bg-stone-50/50" : "border-stone-200 hover:border-stone-300 hover:shadow-md"}`}>
      {selected && (
        <div className="absolute top-3 right-3 w-5 h-5 bg-stone-950 rounded-full flex items-center justify-center shadow-sm">
      ...
      <div className="flex items-center justify-between">
        <span className="text-stone-950 font-bold text-sm">{service.base_price}€</span>
